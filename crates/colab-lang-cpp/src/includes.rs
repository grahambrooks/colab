    ...
    F: FnMut(Node<'_>, Node<'_>),
{
    ...
    walk(&mut cursor, source, wanted, &mut visit);
}

fn walk<F>(cursor: &mut TreeCursor, source: &str, wanted: &str, visit: &mut F)
where
    F: FnMut(Node<'_>, Node<'_>),
{
    ...
    if cursor.goto_first_child() {
        loop {
            walk(cursor, source, wanted, visit);
...
/// Apply `(start, end, replacement)` edits in reverse byte order so
...
impl fmt::Display for IncludeRename {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "cpp::include \"{}\" -> \"{}\"", self.from, self.to)
    }
}
...
impl fmt::Display for IncludeDelete {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "cpp::include \"{}\" -> delete", self.target)
    }
}
...
impl fmt::Display for IncludeEnsure {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "cpp::include \"{}\" -> ensure", self.target)
    }
}
